...
Проект работает на [Django 1.11.1](https://docs.djangoproject.com/en/3.1/releases/1.11/).
...
### Как запускать
файл local.py в папке vshaurme/settings/ и положите туда переменную 'SECRET_KEY' с любым содержимым.
После этого установите зависимости из requirements.txt любым удобным вам образом.
Далее создайте базу данных командой '''python manage.py migrate'''. Вам нужно будет повторять эту команду, если вы будете скачивать обновления нашего кода, в которых базу данных будет меняться.
